...
data DeployResult = DeploySuccess | DeployError
...
deployTag tag = do
  ...
  where
    ...
    notify var (Left ex) = do
      ...
      putMVar var $ LodjurState Idle (DeployEvent tag DeployError : history)
    ...
    renderHistory results =
      table_ $ do
        tr_ $ do
          th_ "Tag"
          th_ "Result"
        forM_ results $ \(DeployEvent tag result) ->
          tr_ $ do
            td_ (toHtml tag)
            case result of
              DeploySuccess -> td_ "Success"
              DeployError -> td_ "Error"
  ...
  modifyState $
    \case
      ...
      LodjurState (Deploying tag) history -> do
        renderHtml $ do
          p_ $ toHtml $ "Already deploying a tag " <> tag
