...
runServer :: Int -> FilePath -> Int -> BS.ByteString -> DbPool -> Core -> IO ()
runServer port staticDir githubAppId webhookSecretToken dbPool core = do
  let key = gitHubKey (pure webhookSecretToken)
      cfg = Config { cfgGithubAppId = githubAppId }
      ctx = AppCtx { appConfig = cfg, appCore = core, appDbPool = dbPool }
  putStrLn $ "Serving on port " ++ show port ++ ", static from " ++ show staticDir
  Warp.run port $
