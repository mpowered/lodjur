...
data JobT f = Job
  { jobId               :: C f UUID
  , jobDescription      :: C f Text
  , jobRevision         :: PrimaryKey RevisionT f
  , jobStartTime        :: C f UTCTime
  , jobFinishTime       :: C f UTCTime
  , jobStatus           :: C f JobStatus
  , jobStartedBy        :: C f Text
  } deriving (Generic, Beamable)
...
instance Table CheckT where
  ...
  primaryKey = CheckKey <$> jobId . checkJob
